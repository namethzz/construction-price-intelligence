  ...
  return (
    <>
      ...
      <section className="card pr-results">
        ...
        <div className="pr-mobile-list">{paginatedMaterials.map((item) => <article key={`mobile-${item.id}`}><div className="pr-item-head"><b>{item.id}</b><div><strong>{item.name}</strong><span>{item.category}</span></div></div><div className="pr-item-price"><div><span>ราคาล่าสุด</span><div className="pr-price-value"><strong>{item.currentPrice === null ? "—" : `฿${formatPrice(item.currentPrice)}`}</strong><Change value={item.changePercent} /></div><small>{item.unit}</small></div><div><span>เดือนก่อน</span><strong>{item.previousPrice === null ? "—" : `฿${formatPrice(item.previousPrice)}`}</strong></div></div><div className="pr-item-source"><span>แหล่งข้อมูล</span><strong>{item.sourceLabel}</strong></div></article>)}</div>
...
const PRICE_STYLES = `
  .pr-source { display:flex; align-items:center; gap:10px; margin-bottom:14px; padding:11px 13px; border-radius:11px; background:rgba(59,130,246,.07); color:#1d4ed8; font-size:12px; } .pr-source svg { flex:0 0 auto; } .pr-source div { min-width:0; flex:1; } .pr-source strong,.pr-source span { display:block; } .pr-source span { margin-top:2px; opacity:.75; } .pr-source a { color:inherit; font-weight:750; text-decoration:none; white-space:nowrap; }
  .pr-stats { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:10px; margin-bottom:14px; } .pr-metric { position:relative; padding:14px !important; } .pr-metric > i { position:absolute; top:12px; right:12px; opacity:.58; } .pr-metric > span,.pr-metric small { display:block; font-size:11px; opacity:.65; } .pr-metric strong { display:block; margin:6px 0 4px; font-size:17px; }
  .pr-filters { display:grid; grid-template-columns:minmax(260px,1.3fr) minmax(180px,.8fr) minmax(180px,.8fr) auto; align-items:end; gap:10px; margin-bottom:14px; } .pr-filters label > span { display:block; margin-bottom:6px; font-size:11px; font-weight:700; opacity:.68; } .pr-filters label > div,.pr-filters select { min-height:44px; border:1px solid rgba(148,163,184,.23); border-radius:9px; background:rgba(148,163,184,.035); color:inherit; }
  ...
  .pr-head { display:flex; align-items:flex-start; justify-content:space-between; gap:12px; margin-bottom:11px; } .pr-head h2 { display:flex; align-items:center; gap:7px; margin:0; font-size:15px; } .pr-head p,.pr-head > span { margin:4px 0 0; font-size:11px; opacity:.65; }
  .pr-table-wrap { overflow-x:auto; } .pr-table-wrap table { width:100%; min-width:850px; border-collapse:collapse; } .pr-table-wrap th { padding:10px 11px; text-align:left; font-size:11px; opacity:.65; background:rgba(148,163,184,.04); } .pr-table-wrap td { padding:11px; border-top:1px solid rgba(148,163,184,.12); font-size:12px; } .pr-table-wrap td.number { font-weight:750; white-space:nowrap; }
  .pr-name { display:flex; align-items:center; gap:9px; min-width:330px; } .pr-name > b { min-width:112px; height:30px; display:grid; place-items:center; flex:0 0 auto; padding:0 7px; border-radius:8px; background:rgba(59,130,246,.1); color:#2563eb; font-size:11px; } .pr-name span strong,.pr-name span small { display:block; } .pr-name span small { margin-top:2px; font-size:10px; opacity:.62; }
  .pr-change { display:inline-flex; align-items:center; gap:3px; font-size:12px; font-weight:750; white-space:nowrap; } .pr-change.up { color:#dc2626; } .pr-change.down { color:#059669; } .pr-change.neutral { opacity:.62; }
  .pr-mobile-list { display:none; } .pr-empty { min-height:140px; display:grid; place-items:center; font-size:12px; opacity:.65; }
  .pr-pagination { display:flex; align-items:center; justify-content:center; gap:10px; margin-top:14px; padding-top:12px; border-top:1px solid rgba(148,163,184,.12); } .pr-pagination span { font-size:11px; opacity:.7; } .pr-pagination button { min-height:44px; padding:0 16px; border:1px solid rgba(148,163,184,.22); border-radius:8px; background:transparent; color:inherit; cursor:pointer; font-weight:700; } .pr-pagination button:disabled { cursor:not-allowed; opacity:.58; }
  ...
  @media (max-width:620px) { .pr-source { align-items:flex-start; flex-wrap:wrap; } .pr-source a { width:100%; padding-left:28px; white-space:normal; } .pr-stats { grid-template-columns:1fr 1fr; } .pr-metric { padding:11px !important; } .pr-metric > span,.pr-metric small { font-size:12px; } .pr-metric strong { font-size:15px; } .pr-filters { grid-template-columns:1fr; padding:14px !important; } .pr-search { grid-column:auto; } .pr-filters label > span { font-size:12px; } .pr-filters input,.pr-filters select { font-size:16px; } .pr-filters button { justify-content:center; } .pr-head { flex-direction:column; } .pr-head p,.pr-head > span { font-size:12px; } .pr-table-wrap { display:none; } .pr-mobile-list { display:grid; gap:9px; } .pr-mobile-list article { padding:12px; border:1px solid rgba(148,163,184,.15); border-radius:10px; } .pr-item-head { display:flex; align-items:flex-start; gap:9px; } .pr-item-head > b { min-width:108px; min-height:31px; display:grid; place-items:center; flex:0 0 auto; padding:0 6px; border-radius:8px; background:rgba(59,130,246,.1); color:#2563eb; font-size:11px; } .pr-item-head > div { min-width:0; flex:1; } .pr-item-head > div strong,.pr-item-head > div span { display:block; } .pr-item-head > div strong { font-size:12px; } .pr-item-head > div span { margin-top:2px; font-size:11px; opacity:.65; } .pr-item-price { display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-top:10px; } .pr-item-price > div { min-width:0; padding:9px; border-radius:8px; background:rgba(148,163,184,.055); } .pr-item-price span,.pr-item-price small { display:block; font-size:11px; opacity:.65; } .pr-item-price strong { display:block; margin:4px 0 2px; font-size:13px; } .pr-price-value { display:flex; align-items:center; justify-content:space-between; gap:6px; } .pr-price-value .pr-change { flex:0 0 auto; margin-top:2px; font-size:11px; } .pr-item-source { margin-top:9px; padding-top:8px; border-top:1px solid rgba(148,163,184,.12); } .pr-item-source span,.pr-item-source strong { display:block; } .pr-item-source span { font-size:11px; opacity:.62; } .pr-item-source strong { margin-top:2px; font-size:11px; } }
  @media (max-width:390px) { .pr-item-head { display:grid; grid-template-columns:1fr; } .pr-item-head > b { min-width:0; max-width:100%; justify-self:start; overflow-wrap:anywhere; } .pr-item-price { grid-template-columns:1fr; } }
