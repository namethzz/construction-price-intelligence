  ...
  return (
    <>
      ...
      <section className="card fc-chart-card">
        ...
        {chartData.length ? <div className="fc-chart"><ResponsiveContainer width="100%" height="100%"><AreaChart data={chartData} margin={{ top: 12, right: 14, left: 2, bottom: 4 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="period" tickLine={false} axisLine={false} minTickGap={28} interval="preserveStartEnd" tick={{ fontSize: 11 }} /><YAxis width={64} tickLine={false} axisLine={false} tickFormatter={(value) => `฿${Number(value).toLocaleString("th-TH", { notation: "compact", maximumFractionDigits: 1 })}`} /><Tooltip formatter={(value, name) => [`฿${formatPrice(value)} ${selected?.unit || ""}`, name === "actual" ? "ราคาย้อนหลัง" : name === "forecast" ? "ผล ML" : name]} /><Area type="monotone" dataKey="actual" stroke="#2563eb" fill="rgba(37,99,235,.10)" strokeWidth={2.5} connectNulls /><Area type="monotone" dataKey="forecast" stroke="#d97706" fill="none" strokeDasharray="7 5" strokeWidth={2.5} connectNulls /></AreaChart></ResponsiveContainer></div> : <EmptyState text="ยังไม่มีประวัติราคาสำหรับวัสดุนี้" />}
      </section>
...
const FORECAST_STYLES = `
  ...
  @media (max-width:560px) { .fc-status { min-height:44px; font-size:12px; } .fc-notice,.fc-change,.fc-model-card > span,.fc-model-card dl div,.fc-summary span,.fc-summary small,.fc-card-head p,.fc-card-head a,.fc-series-list small,.fc-empty span { font-size:12px; } .fc-control-grid { grid-template-columns:1fr; } .fc-controls { padding:15px !important; } .fc-field > div { min-height:46px; } .fc-field input,.fc-field select { font-size:16px; } .fc-target { flex-wrap:wrap; } .fc-target i { display:none; } .fc-target span:nth-of-type(2) { width:100%; margin-top:2px; } .fc-result { gap:15px; padding:17px; } .fc-big-number { font-size:28px; } .fc-summary-grid { grid-template-columns:1fr 1fr; } .fc-summary strong { font-size:15px; } .fc-card-head { align-items:flex-start; flex-direction:column; } .fc-card-head a { min-height:44px; display:flex; align-items:center; } .fc-chart { height:255px; margin-left:-8px; } .fc-series-list article { grid-template-columns:1fr auto; gap:8px; } .fc-series-list > article > div:last-child { grid-column:1/-1; display:flex; justify-content:space-between; text-align:left; } }
