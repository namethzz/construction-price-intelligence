    ...
    <>
      ...
      <div className="ov-main-grid">
        <section className="card ov-chart-card">
          <div className="ov-card-head"><div><h2><TrendingUp size={18} /> แนวโน้มราคา</h2><p>{selected?.name} • {selected?.unit}</p></div></div>
          <div className="ov-selector-row">
            <MaterialPicker materials={materialCatalog} value={selected?.id ?? ""} onChange={setSelectedMaterialId} label="วัสดุสำหรับดูแนวโน้มราคา" />
            <label className="ov-month-control"><span>ช่วงย้อนหลัง</span><div><input inputMode="numeric" type="number" min="1" step="1" value={visibleMonths} onChange={(event) => setVisibleMonths(event.target.value)} /><small>เดือน</small></div></label>
          </div>
...
const OVERVIEW_STYLES = `
  ...
  .ov-selector-row { display:grid; grid-template-columns:minmax(0,1fr) 118px; align-items:end; gap:10px; margin-bottom:15px; } .ov-month-control > span { display:block; margin:0 0 6px 2px; font-size:11px; font-weight:750; opacity:.65; }
  .ov-month-control > div { min-height:72px; display:flex; align-items:center; justify-content:center; gap:5px; border:1px solid rgba(148,163,184,.22); border-radius:15px; background:rgba(148,163,184,.035); } .ov-month-control input { width:55px; border:0; outline:0; background:transparent; color:inherit; font:inherit; font-size:18px; font-weight:850; text-align:right; } .ov-month-control small { font-size:11px; opacity:.65; }
